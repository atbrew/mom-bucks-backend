...
| `type`        | `'LODGE' \| 'WITHDRAW'`               | Direction of the transaction. `LODGE` (credit) increments `child.balance`; `WITHDRAW` (debit) decrements it. Banking model — activity claims and vault unlocks land as `LODGE` rows tagged via the `source` field. **Immutable after create** — rules refuse a post-create flip, which would re-sign the row against `child.balance`. |
| `source`      | `'ACTIVITY' \| 'VAULT_UNLOCK' \| 'VAULT_DEPOSIT'` (optional) | Origin tag on server-written rows. `ACTIVITY` / `VAULT_UNLOCK` are written as `LODGE` credits by `claimActivity` / `unlockVault`; `VAULT_DEPOSIT` is written as a `WITHDRAW` debit by `depositToVault`. Parent-initiated rows omit this field. `onTransactionCreate` skips balance recompute on any row with `source` set — the callable already applied the balance delta in the same transaction that wrote the row. Client-path writes are refused by `firestore.rules` (creates denied if `source` is present; updates blocked by `txnCoreUnchanged`); Admin SDK bypasses rules, so callables can still set it. **Immutable after create**. |
