...
- `children/{childId}/transactions/**` additionally enforces an
  **overspend guard on create**: a `WITHDRAW` whose `amount` exceeds
  the parent child's current `balance` is rejected synchronously. This
  has to live in the rules because `onTransactionCreate` (#15) fires
  after the doc has already been written — refusal at the trigger
  can't un-do the write. The trigger's clamp-at-zero path remains as
  defense-in-depth for Admin-SDK writers (which bypass rules) and
  concurrent-WITHDRAW races.
