...
| `source`      | `'ACTIVITY' \| 'VAULT_UNLOCK'` (optional) | Origin tag on server-written `LODGE` rows. `ACTIVITY` is written by `claimActivity`; `VAULT_UNLOCK` is written by `unlockVault`. Parent-initiated lodges omit this field. `onTransactionCreate` skips balance recompute on any `LODGE` with `source` set — the callable already applied the balance bump in the same transaction that wrote the row. **Immutable after create**. |
