...
Exposes the bug that `mb auth create-account --name Alice` does not
...
def test_create_account_writes_display_name_to_user_doc():
    """After create-account --name Alice, the CLI must have written
...
def test_delete_account_deletes_auth_user_and_user_doc():
    """With --yes to skip the prompt, delete-account must:
